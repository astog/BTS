...
local function PopulateCheckBox(control, setting_name)
    ...
    if (current_value == nil) then
        if (GameInfo.BTS_Settings[setting_name]) then --LY Checks if this setting has a default state defined in the database
            if (GameInfo.BTS_Settings[setting_name].Value == 0) then --because 0 is true in Lua
                current_value = false;
            else
                current_value = true;
            end
...
local function Initialize()
    ...
    PopulateCheckBox(Controls.ApproximateTraderPathCheckbox, "BTS_ApproximateTraderPath")
    PopulateCheckBox(Controls.ShowSortPrioritiesCheckbox, "BTS_ShowSortPriorities")
